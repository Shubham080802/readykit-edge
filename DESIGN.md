...
# ReadyKit Edge extension

> The document above is vendored unmodified from
> [VoltAgent/awesome-design-md](https://github.com/VoltAgent/awesome-design-md)
> (`design-md/linear.app/DESIGN.md`), a curated collection of DESIGN.md files
> extracted from real products. It is the visual language for the ReadyKit
> Edge operator console.
>
> Everything below is ReadyKit's own extension. Where the two disagree, this
> section wins — but it deliberately disagrees as little as possible.

## Why this language

The console is a dense, technical, dark-canvas instrument that lives on a wall
in a workshop or a staging area. Linear's system is the closest fit in the
collection: near-black canvas, a surface ladder that carries hierarchy without
shadow, hairline borders, and a single accent used scarcely. A marketing-led
system (SpaceX, Nike, Tesla) would have given atmosphere at the cost of the
data density this screen needs.

Two deviations, both forced by the domain:

**Mono is promoted.** Linear reserves `{typography.mono}` for code inside
product screenshots. Here it carries confidences, frame digests, sequence
numbers, and timestamps — figures an operator compares down a column. Tabular
numerals matter more than voice consistency.

**A second and third semantic colour.** Linear ships exactly one semantic
colour (`{colors.semantic-success}`) because its marketing canvas needs no
more. A verdict display needs three, and they must be distinguishable at a
glance from across a room.

## Verdict semantics

These are the only colours added to the palette. Each is used for exactly one
Verdict and nothing else — they never appear decoratively.

| Token | Hex | Verdict | Meaning |
|---|---|---|---|
| `verdict-pass` | `#27a644` | `PASS` | Latch released. Linear's own success green, unchanged. |
| `verdict-fail` | `#e5484d` | `FAIL` | A positive finding of non-compliance. Latch engaged. |
| `verdict-hold` | `#e2a336` | `INDETERMINATE` | Compliance could not be established. Latch engaged. |
| `verdict-stale` | `#8a8f98` | — | The Host Link has gone quiet. Reuses `{colors.ink-subtle}`. |

Each verdict colour also has a 12%-alpha wash for the tint behind its panel and
a 30%-alpha for its border. Never use a verdict colour on a surface, a button,
or a chart series — only on the verdict itself and the items that caused it.

### Amber, not orange

`INDETERMINATE` is the verdict most likely to be misread as "nearly a pass", so
it must not look like a warmer green. `#e2a336` sits far enough from
`verdict-pass` in hue and far enough from `verdict-fail` in saturation to be
unambiguous, and it survives the common red-green colour vision deficiencies
that would otherwise collapse pass and fail into each other.

**Colour is never the only signal.** Every verdict carries a glyph
(`+` found · `−` absent · `!` damaged · `?` unreadable) and an explicit text
label. An operator who cannot distinguish the hues still reads the state.

## Layout

The console is one screen, no scrolling on a 1440×900 wall display, three
regions:

- **Verdict banner** — `{typography.display-md}`, the verdict word, the latch
  state, and the reason. Readable across a room.
- **Manifest checklist** — one row per Required Item: glyph, label, presence,
  confidence in mono. The row tints with its own verdict colour only when it
  is the cause of a failure.
- **Telemetry and audit** — latch, indicator, link freshness, last command and
  its acknowledgement; then the Inspection Record tail.

## Type

Linear's typeface is not publicly distributed. The console uses **Inter** at
500/600 for display and **JetBrains Mono** at 400 for figures, the substitutes
named in the document above. Both are loaded from Google Fonts with full
system-stack fallbacks, so the console renders correctly on an air-gapped
machine that can reach neither.
